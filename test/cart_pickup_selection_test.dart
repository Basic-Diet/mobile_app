...
void main() {
  group('Cart pickup selection', () {
    ...
    test(
      'allows checkout only after branch and pickup window are selected',
      () {
        final state = CartLoaded(
          items: const [_cartItem],
    ...
    test(
      'clears selected pickup window when branch changes without that window',
      () async {
        final bloc = CartBloc();
        ...
        final selectedWindow = bloc.stream.firstWhere(
          (state) =>
              state is CartLoaded &&
              state.selectedPickupWindow == '12:00-14:00',
        );
    ...
    test(
      'serializes selected pickup branch and window into order requests',
      () {
...
const _cartItem = CartItem(productId: 'meal-1', name: 'Chicken Meal', qty: 1);
